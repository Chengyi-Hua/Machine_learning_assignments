...
# first mean
climate_c = climate - np.mean(climate, axis=0)
np.mean(climate_c, axis=0)  # check
# then std dev
X = climate_c / np.std(climate_c, axis=0)
np.std(X, axis=0)  # check
# %%
# Plotting
...
# %%
def plot_matrix(
    M, lim=None, cmap="PiYG", labels="{:.3g}", rownames=None, colnames=None, fontsize=16,  **kwargs
):
